...
module AwsLambdaRuntimeInterfaceClient
  ...
  class LambdaRunner
    ...
    def run_user_code(lambda_invocation_request)
      ...
      LambdaLogger.log_error(
        exception: e,
        message: 'Error raised from handler method',
      )
      send_error_response(lambda_invocation_request, e)
    rescue LambdaErrors::LambdaHandlerCriticalException => e
      LambdaLogger.log_error(
        exception: e,
        message: 'Critical exception from handler',
      )
    ...
    def send_init_error_to_server(err)
      ...
      LambdaLogger.log_error(
        exception: ex,
        message: "Init error when loading handler #{@env_handler}",
      )
      @lambda_server.send_init_error(
        error_object: ex.to_lambda_response,
        error: ex,
      )
    end
  ...
  class TelemetryLoggingHelper
    ...
    def initialize(telemetry_log_fd, path_to_fd = '/proc/self/fd/')
      ...
      AwsLambdaRuntimeInterfaceClient::TelemetryLoggingHelper.telemetry_log_fd_file =
        File.open(fd, 'wb')
      AwsLambdaRuntimeInterfaceClient::TelemetryLoggingHelper
        .telemetry_log_fd_file.sync =
        true

      AwsLambdaRuntimeInterfaceClient::TelemetryLoggingHelper.telemetry_log_sink =
        TelemetryLogSink.new(
          file:
            AwsLambdaRuntimeInterfaceClient::TelemetryLoggingHelper
